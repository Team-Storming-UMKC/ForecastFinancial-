...
export default function TransactionCharts({ refreshKey }: TransactionChartsProps) {
    ...
    const timeSeries = React.useMemo(() => {
        ...
        for (const t of filtered) {
            ...
            map.get(key)!.spending += Math.abs(amount);
        }
    ...
    const topMerchants = React.useMemo(() => {
        ...
        for (const t of filtered) {
            ...
            map.set(m, (map.get(m) ?? 0) + Math.abs(amount));
        }
        return Array.from(map.entries())
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value)
        ...
        <Box sx={rootSx}>
            <Box sx={contentSx}>
                ...
                {loading ? (
                    ...
                    <Stack spacing={2.25}>
                        <StatsRow totalSpending={totalSpending} transactionCount={filtered.filter((t) => Number(t.amount) < 0).length} />
